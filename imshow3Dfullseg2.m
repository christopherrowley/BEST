function  imshow3Dfullseg2( Img, disprange, seg,seg2)
...
% C.R. Aug 2019- modified to take in two segmentation images, one for WM
% and one for GM. 
...
if verLessThan('matlab', '8')
    ...
    Seg2Sg = flipdim(permute(seg2, [3 1 2 4]),1);   % Sagittal view image
    Seg2Cr = flipdim(permute(seg2, [3 2 1 4]),1);   % Coronal view image
    ...
    Seg2Sg = flip(permute(seg2, [3 1 2 4]),1);   % Sagittal view image
    Seg2Cr = flip(permute(seg2, [3 2 1 4]),1);   % Coronal view image
end
...
%         end
%         % segmentation #2
%         boundaries = bwboundaries(squeeze(seg2(:,:,S)));
%         for k = 1:size(boundaries,1)
%             bound = boundaries{k};
...
% -=< Coronal view callback function >=-
    ...
    function SegAdd(segm)
            boundaries = bwboundaries(squeeze(segm(:,:,S)));
            for k = 1:size(boundaries,1)
                bound = boundaries{k};
